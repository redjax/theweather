...
source "$THIS_DIR/build_api-server.sh"
...
run_container() {
    ...
    if ! "${cmd[@]}"; then
        echo ""
        echo "[ERROR] Failed to run api-server container"
        echo ""
        exit 1
    else
        echo ""
        echo "api-server container started successfully. Check health with docker ps -a"
        echo ""
    fi
}
